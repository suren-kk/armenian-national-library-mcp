...
Set `NLA_DRIFT_CHECK_ACCESS=true` to also issue bounded anonymous `GET` probes for non-templated records with a declared live expectation. Probes request a single result and accept at most 64 KiB. Relations whose base URLs require a resource identifier, operation-specific parameters, authenticated context, or otherwise do not produce an authoritative access decision are explicitly listed in `accessChecksNotProbeable`; these approved exclusions do not fail the check. Any unexpected direct-probe failure appears in `accessChecksSkipped` and now counts as drift, so inconclusive coverage cannot produce a clean nightly report. Public-to-protected and protected-to-public changes are also reported as drift.
