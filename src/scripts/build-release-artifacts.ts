...
function main(): void {
  ...
  const sbomFilename = `nla-mcp-${packageManifest.version}.cdx.json`;
  writeFileSync(
  ...
  const sourceFilename = `nla-mcp-${packageManifest.version}-source.tar.gz`;
  run("git", [
    "archive",
    "--format=tar.gz",
    `--prefix=nla-mcp-${packageManifest.version}/`,
